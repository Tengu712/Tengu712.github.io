...
title: "MVCからReduxまで"
genre: "tech"
tags: ["arch", "elm", "flux", "mvc", "mvp", "mvvm", "redux", "tca", "vue"]
date: "2025/08/29"
---
...
_本記事では[1][2]のように参考文献を、[AFAIK]で参考文献を提示できないがどう考えてもそうであることを、[IMO]で間違っているかもしれないが個人的に考えられることを示します。_
...
本記事では参考文献を正しい情報とし、そこに載っている情報のみを扱う。
...
例えば大きな橋の建設や論文のアイデア等なんでも良い。

ここで注目すべきはまだObserverパターンが使われていない点である。
そのため、ViewがModelとやりとりを行っている。
つまり、ユーザ→Controller→View→Model→Viewという流れを持つ。
...
それと色との対応マップを持ったテキストフィールド・あるいはラベル・あるいはそれらの複合コンポーネントのサブクラスを定義するのが良い。
...
かといって、ウィンドウが別れているために、前述のようにサブクラスを定義して解決することもできない。
